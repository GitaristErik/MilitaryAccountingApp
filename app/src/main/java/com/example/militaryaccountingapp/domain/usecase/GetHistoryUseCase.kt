...
//        page = page,
        ...
        val res = list.map {
            ...
            val data = (dataRes as? Results.Success) ?: return@safetyResultWrapper Results.Failure(
                (dataRes as? Results.Failure)?.throwable
                    ?: return@safetyResultWrapper Results.Failure(
                        Exception("dataRes is unknown")
                    )
            )
            Triple(it, data.data, user.data)
        }
        Results.Success(res)
    }
}